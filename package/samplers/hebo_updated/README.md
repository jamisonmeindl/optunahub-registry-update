...
- `HEBOSampler(search_space: dict[str, BaseDistribution] | None = None, *, seed: int | None = None, constant_liar: bool = False, independent_sampler: BaseSampler | None = None, num_obj: int = 1, rand_sample: int | None = None, model_name: str | None = None, track_surrogate_predictions: bool = False, failure_constraint_model: bool = False)`
  ...
  - `num_obj`: If `>1`, use the multi-objective version of HEBO (GeneralBO) as the backend. Default is `1`.
  - `rand_sample`: Override the number of initial random suggestions HEBO uses before relying on the surrogate model. If you observe roughly `d` random trials in `d` dimensions, set this smaller (for example `rand_sample=1`) to reduce warmup.
  - `model_name`: Override the HEBO surrogate backend model (for example `"gp"`, `"rf"`, `"deep_ensemble"`, or `"catboost"`), if supported by the installed HEBO version.
  - `track_surrogate_predictions`: If `True`, store the surrogate mean/std for each candidate proposed by HEBO (single-objective mode).
  - `failure_constraint_model`: If `True`, infer one hidden feasibility constraint from trial outcomes only (complete + finite objective -> feasible, failed/invalid -> infeasible), and pass it through HEBO's internal constrained optimizer path.
